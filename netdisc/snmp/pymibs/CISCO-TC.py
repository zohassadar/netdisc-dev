...
if mibBuilder.loadTexts:
    ciscoTextualConventions.setLastUpdated("201506090000Z")
if mibBuilder.loadTexts:
    ciscoTextualConventions.setOrganization("Cisco Systems, Inc.")


class Layer2Cos(TextualConvention, Integer32):
    reference = (
        "IEEE 802.1D, 2004 Edition, Annex G User priorities and traffic classes."
    )
    status = "current"
...
class CiscoLocationClass(TextualConvention, Integer32):
    status = "current"
    subtypeSpec = Integer32.subtypeSpec + ConstraintsUnion(
        SingleValueConstraint(1, 2, 3, 4, 5, 6, 7, 8)
    )
    namedValues = NamedValues(
        ("chassis", 1),
        ("shelf", 2),
...
mibBuilder.exportSymbols(
    ...
    PYSNMP_MODULE_ID=ciscoTextualConventions,
